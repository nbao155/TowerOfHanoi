...
public class HanoiTower {
	...
	public void playGame(String[] args) {
		...
		while (! isDone()) {
			printTowers();
			int from = 0;//pt.getInt("\nMove disk FROM peg", 0, 2);
			int to = 0;//pt.getInt("          TO   peg", 0, 2);
			if (from == to)
	...
	public void solvePuzzle(String[] args) {
		...
		solveHanoi(0, 1, Integer.parseInt(args[0]));
	}
	
	public void solveHanoi(int from, int to, int levels){
		int newTo = 0;
		if(levels==0){
		}
		else{
			...
			solveHanoi(from, third, levels - 1);
			moveDisk(from, to);
			solveHanoi(third, to, levels - 1);
		}
	}
